...
* **API 서버 (Swagger)**: [http://113.198.66.75:10033/docs](http://113.198.66.75:10033/docs) (8080 포트 매핑)
...
### 1) 환경 변수 설정
프로젝트 루트에 `.env` 파일을 생성하세요. **(주의: 보안상 깃허브에는 `.env.example`만 업로드해야 합니다.)**
```bash
...
### 5. 테스트 계정 정보 (RBAC)
역할 (Role)아이디 (Email)비밀번호권한 범위일반 사용자user@example.compassword123리뷰 작성, 리스트 관리 등관리자admin@example.compassword123유저 목록 조회, 영화 삭제, 시스템 통계

### 6. 주요 엔드포인트 요약 (총 30개 이상)
